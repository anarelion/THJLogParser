...
namespace EQLogParser
{
    class PlayerManager
    {
        ...
        // static data
        private readonly ConcurrentDictionary<SpellClass, string> ClassNames =
            new ConcurrentDictionary<SpellClass, string>();
        private readonly ConcurrentDictionary<string, SpellClass> ClassesByName =
            new ConcurrentDictionary<string, SpellClass>();
        private readonly ConcurrentDictionary<string, byte> GameGeneratedPets =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<string, byte> SecondPerson =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<string, byte> ThirdPerson =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<string, string> PetToPlayer =
            new ConcurrentDictionary<string, string>();
        private readonly ConcurrentDictionary<string, SpellClassCounter> PlayerToClass =
            new ConcurrentDictionary<string, SpellClassCounter>();
        private readonly ConcurrentDictionary<string, byte> TakenPetOrPlayerAction =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<string, byte> VerifiedPets =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<string, double> VerifiedPlayers =
            new ConcurrentDictionary<string, double>();
        private readonly ConcurrentDictionary<string, byte> Mercs =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<string, byte> DoTClasses =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<string, byte> CharmPets =
            new ConcurrentDictionary<string, byte>();
        ...
        private PlayerManager()
        {
            ...
            {
                var name = EQLogParser.Resource.ResourceManager.GetString(
                    Enum.GetName(typeof(SpellClass), item),
            ...
            ConfigUtil
                .ReadList(@"data\petnames.txt")
                .ForEach(line => GameGeneratedPets[line.TrimEnd()] = 1);
        ...
        internal bool IsCharmPet(string name) =>
            !string.IsNullOrEmpty(name) && CharmPets.ContainsKey(name);

        internal bool IsDoTClass(string name) =>
            !string.IsNullOrEmpty(name) && DoTClasses.ContainsKey(name);

        internal bool IsVerifiedPlayer(string name) =>
            !string.IsNullOrEmpty(name)
            && (
                name == Labels.UNASSIGNED
                || SecondPerson.ContainsKey(name)
                || ThirdPerson.ContainsKey(name)
                || VerifiedPlayers.ContainsKey(name)
            );

        internal bool IsPetOrPlayerOrMerc(string name) =>
            !string.IsNullOrEmpty(name)
            && (
                IsVerifiedPlayer(name)
                || IsVerifiedPet(name)
                || IsMerc(name)
                || TakenPetOrPlayerAction.ContainsKey(name)
            );

        internal bool IsPetOrPlayerOrSpell(string name) =>
            IsPetOrPlayerOrMerc(name) || DataManager.Instance.IsPlayerSpell(name);

        internal List<string> GetClassList(bool withNull = false) =>
            withNull ? SortedClassListWithNull : SortedClassList;

        internal bool IsMerc(string name) =>
            Mercs.TryGetValue(TextFormatUtils.ToUpper(name), out _);
        ...
        internal void AddPetOrPlayerAction(string name)
        {
            if (
                !IsVerifiedPlayer(name)
                && !IsVerifiedPet(name)
                && TakenPetOrPlayerAction.TryAdd(name, 1)
            )
            {
            ...
            if (!string.IsNullOrEmpty(pet) && !string.IsNullOrEmpty(player))
            {
                if (!PetToPlayer.ContainsKey(pet) || PetToPlayer[pet] != player)
                {
                    if (!IsVerifiedPlayer(pet))
                    {
                        ...
                        EventsNewPetMapping?.Invoke(
                            this,
                            new PetMapping { Pet = pet, Owner = player }
                        );
        ...
        internal void AddVerifiedPet(string name)
        {
            if (!string.IsNullOrEmpty(name) && !VerifiedPets.ContainsKey(name))
            {
                ...
                {
                    if (
                        !name.EndsWith("`s pet", StringComparison.OrdinalIgnoreCase)
                        && !name.EndsWith("`s ward", StringComparison.OrdinalIgnoreCase)
                        && !name.EndsWith("`s warder", StringComparison.OrdinalIgnoreCase)
                        && !MainWindow.IsIgnoreCharmPetsEnabled
                    )
                    {
                        CharmPets[name] = 1;
                    }
        ...
        internal string GetPlayerClass(string name)
        {
            ...
            if (
                !string.IsNullOrEmpty(name)
                && PlayerToClass.TryGetValue(name, out SpellClassCounter counter)
            )
            {
        ...
        {
            BitmapImage icon = PlayerManagerIcons.UNK_ICON;

            switch (GetPlayerClassEnum(name))
            {
                case SpellClass.BER:
                    icon = PlayerManagerIcons.BER_ICON;
                    break;
                case SpellClass.BRD:
                    icon = PlayerManagerIcons.BRD_ICON;
                    break;
                case SpellClass.BST:
                    icon = PlayerManagerIcons.BST_ICON;
                    break;
                case SpellClass.CLR:
                    icon = PlayerManagerIcons.CLR_ICON;
                    break;
                case SpellClass.DRU:
                    icon = PlayerManagerIcons.DRU_ICON;
                    break;
                case SpellClass.ENC:
                    icon = PlayerManagerIcons.ENC_ICON;
                    break;
                case SpellClass.MAG:
                    icon = PlayerManagerIcons.MAG_ICON;
                    break;
                case SpellClass.MNK:
                    icon = PlayerManagerIcons.MNK_ICON;
                    break;
                case SpellClass.NEC:
                    icon = PlayerManagerIcons.NEC_ICON;
                    break;
                case SpellClass.PAL:
                    icon = PlayerManagerIcons.PAL_ICON;
                    break;
                case SpellClass.RNG:
                    icon = PlayerManagerIcons.RNG_ICON;
                    break;
                case SpellClass.ROG:
                    icon = PlayerManagerIcons.ROG_ICON;
                    break;
                case SpellClass.SHD:
                    icon = PlayerManagerIcons.SHD_ICON;
                    break;
                case SpellClass.SHM:
                    icon = PlayerManagerIcons.SHM_ICON;
                    break;
                case SpellClass.WAR:
                    icon = PlayerManagerIcons.WAR_ICON;
                    break;
                case SpellClass.WIZ:
                    icon = PlayerManagerIcons.WIZ_ICON;
                    break;
            }
        ...
        internal void Save()
        {
            if (PetMappingUpdated)
            {
                lock (PetToPlayer)
                {
                    var filtered = PetToPlayer.Where(keypair =>
                        !GameGeneratedPets.ContainsKey(keypair.Key)
                        && IsPossiblePlayerName(keypair.Key)
                ...
                lock (VerifiedPlayers)
                {
                    ...
                    foreach (var keypair in VerifiedPlayers)
                    {
                        if (!string.IsNullOrEmpty(keypair.Key) && IsPossiblePlayerName(keypair.Key))
                        {
                            if (
                                keypair.Value != 0
                                && (now - DateUtil.FromDouble(keypair.Value)).TotalDays < 300
                            )
                            {
                                var output = keypair.Key + "=" + Math.Round(keypair.Value);
                                if (
                                    PlayerToClass.TryGetValue(
                                        keypair.Key,
                                        out SpellClassCounter value
                                    )
                                    && value.CurrentMax == long.MaxValue
                                    && ClassNames.TryGetValue(
                                        value.CurrentClass,
        ...
        internal static int FindPossiblePlayerName(
            string part,
            out bool isCrossServer,
            int start = 0,
            int stop = -1,
            char end = char.MaxValue
        )
        {
            isCrossServer = false;
            int dotCount = 0;
        ...
        internal static bool IsPossiblePlayerName(string part, int stop = -1) =>
            FindPossiblePlayerName(part, out bool _, 0, stop) > -1;
            ...
            if (values.Length > 0)
            {
                foreach (var value in values)
                {
                    if (!string.IsNullOrEmpty(value) && value.Length >= 2)
                    {
                        ...
                        dict[
                            char.ToUpper(value[0], CultureInfo.CurrentCulture) + value.Substring(1)
                        ] = 1;
                    }
                }
            }
