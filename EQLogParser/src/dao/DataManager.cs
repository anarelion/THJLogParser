...
namespace EQLogParser
{
    ...
    class DataManager
    {
        ...
        private readonly List<string> AdpsKeys = new List<string>
        {
            "#DoTCritRate",
            "#NukeCritRate",
        };
        private readonly Dictionary<string, Dictionary<string, uint>> AdpsActive =
            new Dictionary<string, Dictionary<string, uint>>();
        private readonly Dictionary<string, Dictionary<string, uint>> AdpsValues =
            new Dictionary<string, Dictionary<string, uint>>();
        private readonly Dictionary<string, HashSet<SpellData>> AdpsLandsOn =
            new Dictionary<string, HashSet<SpellData>>();
        private readonly Dictionary<string, HashSet<SpellData>> AdpsWearOff =
            new Dictionary<string, HashSet<SpellData>>();
        private readonly Dictionary<string, List<SpellData>> SpellsNameDB =
            new Dictionary<string, List<SpellData>>();
        ...
        private readonly ConcurrentDictionary<string, byte> AllNpcs =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<
            string,
            Dictionary<SpellResist, ResistCount>
        > NpcResistStats = new ConcurrentDictionary<string, Dictionary<SpellResist, ResistCount>>();
        private readonly ConcurrentDictionary<string, TotalCount> NpcTotalSpellCounts =
            new ConcurrentDictionary<string, TotalCount>();
        private readonly ConcurrentDictionary<string, SpellData> SpellsAbbrvDB =
            new ConcurrentDictionary<string, SpellData>();
        private readonly ConcurrentDictionary<string, SpellClass> SpellsToClass =
            new ConcurrentDictionary<string, SpellClass>();

        private readonly ConcurrentDictionary<string, Fight> ActiveFights =
            new ConcurrentDictionary<string, Fight>();
        private readonly ConcurrentDictionary<string, byte> LifetimeFights =
            new ConcurrentDictionary<string, byte>();
        private readonly ConcurrentDictionary<long, Fight> OverlayFights =
            new ConcurrentDictionary<long, Fight>();

        private readonly ConcurrentDictionary<string, string> SpellAbbrvCache =
            new ConcurrentDictionary<string, string>();
        private readonly ConcurrentDictionary<string, string> RanksCache =
            new ConcurrentDictionary<string, string>();
        private readonly ConcurrentDictionary<string, string> TitleToClass =
            new ConcurrentDictionary<string, string>();
        ...
        private DataManager()
        {
            DictionaryUniqueListHelper<string, SpellData> helper =
                new DictionaryUniqueListHelper<string, SpellData>();
            ...
            Enumerable
                .Range(1, 9)
                .ToList()
                .ForEach(r => RanksCache[r.ToString(CultureInfo.CurrentCulture)] = "");
            Enumerable
                .Range(1, 200)
                .ToList()
                .ForEach(r => RanksCache[TextFormatUtils.IntToRoman(r)] = "");
            ...
            ConfigUtil
                .ReadList(@"data\oldspells.txt")
                .ForEach(line => OldSpellNamesDB[line] = true);
                ...
                try
                {
                    ...
                    if (spellData != null)
                    {
                        ...
                        if (!SpellsAbbrvDB.ContainsKey(spellData.NameAbbrv))
                        {
                            ...
                            string.Compare(
                                SpellsAbbrvDB[spellData.NameAbbrv].Name,
                                spellData.Name,
                                true,
                        ...
                        if (
                            !string.IsNullOrEmpty(spellData.LandsOnOther)
                            && (spellData.Adps > 0 || spellData.IsBeneficial)
                        )
                        {
                            BuildSpellPath(
                                spellData.LandsOnOther.Trim().Split(' ').ToList(),
                                LandsOnOtherTree,
                                spellData
                            );
                        }
                        ...
                        if (
                            !string.IsNullOrEmpty(spellData.LandsOnYou)
                            && (spellData.Adps > 0 || spellData.IsBeneficial)
                        )
                        {
                            BuildSpellPath(
                                spellData.LandsOnYou.Trim().Split(' ').ToList(),
                                LandsOnYouTree,
                                spellData
                            );
                        }
                        ...
                        if (
                            !string.IsNullOrEmpty(spellData.WearOff)
                            && (spellData.Adps > 0 || spellData.IsBeneficial)
                        )
                        {
                            BuildSpellPath(
                                spellData.WearOff.Trim().Split(' ').ToList(),
                                WearOffTree,
                                spellData
                            );
                        }
            ...
            spellList.ForEach(spell =>
            {
                ...
                if (
                    ...
                    && spell.Proc == 0
                    && (
                        tgt == SpellTarget.SELF
                        || tgt == SpellTarget.SINGLETARGET
                        || tgt == SpellTarget.LOS
                        || spell.Rank > 1
                    )
                ...
                )
                {
                    ...
                    if (
                        spell.Name.IndexOf("Illusion", StringComparison.OrdinalIgnoreCase) == -1
                        ...
                        && spell.Name.IndexOf("Call of Fire", StringComparison.OrdinalIgnoreCase)
                            == -1
                    )
                    {
                ...
                )
                {
                    if (trimmed[0] != '#' && !string.IsNullOrEmpty(key))
                    {
                        if (trimmed.Split('|') is string[] multiple && multiple.Length > 0)
                        {
                            ...
                            {
                                if (
                                    spellLine.Split('=') is string[] list
                                    && list.Length == 2
                                    && uint.TryParse(list[1], out uint rate)
                                )
                                {
                                    if (GetAdpsByName(list[0]) is SpellData spellData)
                                    {
                                        ...
                                        if (
                                            !AdpsWearOff.TryGetValue(
                                                spellData.WearOff,
                                                out HashSet<SpellData> wearOffList
                                            )
                                        )
                                        {
                                            AdpsWearOff[spellData.WearOff] =
                                                new HashSet<SpellData>();
                                        }

                                        AdpsWearOff[spellData.WearOff].Add(spellData);

                                        if (
                                            !AdpsLandsOn.TryGetValue(
                                                spellData.LandsOnYou,
                                                out HashSet<SpellData> landsOnList
                                            )
                                        )
                                        {
                                            AdpsLandsOn[spellData.LandsOnYou] =
                                                new HashSet<SpellData>();
                                        }
            ...
            PlayerManager.Instance.EventsNewTakenPetOrPlayerAction += (sender, name) =>
                RemoveFight(name);
        ...
        internal void AddDeathRecord(DeathRecord record, double beginTime) =>
            Helpers.AddAction(AllDeathBlocks, record, beginTime);

        internal void AddMiscRecord(IAction action, double beginTime) =>
            Helpers.AddAction(AllMiscBlocks, action, beginTime);
        ...
        internal string GetClassFromTitle(string title) =>
            TitleToClass.ContainsKey(title) ? TitleToClass[title] : null;

        internal List<ActionBlock> GetCastsDuring(double beginTime, double endTime) =>
            SearchActions(AllSpellCastBlocks, beginTime, endTime);

        internal List<ActionBlock> GetDeathsDuring(double beginTime, double endTime) =>
            SearchActions(AllDeathBlocks, beginTime, endTime);

        internal List<ActionBlock> GetHealsDuring(double beginTime, double endTime) =>
            SearchActions(AllHealBlocks, beginTime, endTime);

        internal List<ActionBlock> GetMiscDuring(double beginTime, double endTime) =>
            SearchActions(AllMiscBlocks, beginTime, endTime);

        internal ConcurrentDictionary<
            string,
            Dictionary<SpellResist, ResistCount>
        > GetNpcResistStats() => NpcResistStats;

        internal ConcurrentDictionary<string, TotalCount> GetNpcTotalSpellCounts() =>
            NpcTotalSpellCounts;

        internal List<ActionBlock> GetResistsDuring(double beginTime, double endTime) =>
            SearchActions(AllResistBlocks, beginTime, endTime);

        internal List<ActionBlock> GetReceivedSpellsDuring(double beginTime, double endTime) =>
            SearchActions(AllReceivedSpellBlocks, beginTime, endTime);

        internal SpellData GetSpellByAbbrv(string abbrv) =>
            (
                !string.IsNullOrEmpty(abbrv)
                && abbrv != Labels.UNKSPELL
                && SpellsAbbrvDB.ContainsKey(abbrv)
            )
                ? SpellsAbbrvDB[abbrv]
                : null;

        internal bool IsKnownNpc(string npc) =>
            !string.IsNullOrEmpty(npc)
            && AllNpcs.ContainsKey(npc.ToLower(CultureInfo.CurrentCulture));
        ...
        internal void AddLootRecord(LootRecord record, double beginTime)
        {
            ...
            {
                var found = AssignedLoot.FindLastIndex(item =>
                    item.Player == record.Player && item.Item == record.Item
                );
                if (found > -1)
                {
                    ...
                    {
                        found = block.Actions.FindLastIndex(item =>
                            item is LootRecord loot
                            && loot.Player == record.Player
        ...
        internal SpellData GetDamagingSpellByName(string name, bool bPC = false)
        {
            ...
            if (
                ...
                && name != Labels.UNKSPELL
                && SpellsNameDB.TryGetValue(name, out List<SpellData> spellList)
            )
            {
            ...
            )
            {
                if (spellList.Count <= 10)
                {
                    foreach (ref SpellData spell in spellList.ToArray().AsSpan())
                    {
                        if (
                            spellData == null
                            || (spellData.Level < spell.Level && spell.Level <= 250)
                            || (spellData.Level > 250 && spell.Level <= 250)
                        )
                        {
                            spellData = spell;
                        }
        ...
        {
            for (
                int i = AllSpellCastBlocks.Count - 1;
                i >= 0 && beginTime - AllSpellCastBlocks[i].BeginTime <= 5;
                i--
            )
            {
                int index = AllSpellCastBlocks[i]
                    .Actions.FindLastIndex(action => action is SpellCast sc && sc.Caster == player);
        ...
        internal SpellTreeResult GetLandsOnYou(List<string> data)
        {
            ...
            if (found.DataIndex == 0 && found.SpellData.Count > 0)
            {
                ...
                if (
                    AdpsLandsOn.TryGetValue(
                        found.SpellData[0].LandsOnYou,
                        out HashSet<SpellData> spellDataSet
                    )
                    && spellDataSet.Count > 0
                )
                {
                    var spellData =
                        spellDataSet.Count == 1
                            ? spellDataSet.First()
                            : FindPreviousCast(ConfigUtil.PlayerName, spellDataSet.ToList(), true);
        ...
        internal void UpdateNpcSpellResistStats(
            string npc,
            SpellResist resist,
            bool resisted = false
        )
        {
            ...
            {
                if (
                    !NpcResistStats.TryGetValue(npc, out Dictionary<SpellResist, ResistCount> stats)
                )
                {
                ...
                {
                    stats[resist] = resisted
                        ? new ResistCount { Resisted = 1 }
                        : new ResistCount { Landed = 1 };
                }
                else
                {
                    if (resisted)
            ...
            if (!string.IsNullOrEmpty(line))
            {
                ...
                else if (data.Length >= 11)
                {
                    ...
                    spellData = new SpellData
                    {
                        ...
                        Resist = (EQLogParser.SpellResist)
                            int.Parse(data[10], CultureInfo.CurrentCulture),
        ...
        private SpellData FindPreviousCast(
            string player,
        ...
        )
        {
            if (LastSpellIndex > -1)
            {
                ...
                for (
                    int i = LastSpellIndex;
                    i >= 0 && AllSpellCastBlocks[i].BeginTime >= endTime;
                    i--
                )
                {
                    ...
                    {
                        if (
                            AllSpellCastBlocks[i].Actions[j] is SpellCast cast
                            && !cast.Interrupted
                            && output.Find(spellData =>
                                (spellData.Target != (int)SpellTarget.SELF || cast.Caster == player)
                                && spellData.Name == cast.Spell
                                && (!isAdps || spellData.Adps > 0)
                            )
                                is SpellData found
                        )
                        {
        ...
        private List<SpellData> FindByLandsOn(string player, List<SpellData> output)
        {
            ...
            else if (output.Count > 1)
            {
                ...
                if (foundSpellData == null)
                {
                    ...
                    result =
                        (output.Distinct(AbbrvComparer).Count() == 1)
                            ? new List<SpellData> { output.First() }
                            : output;
                }
                else
                {
        ...
        internal static bool ResolveSpellAmbiguity(ReceivedSpell spell, out SpellData replaced)
        {
            ...
            if (spell.Ambiguity.Count < 30)
            {
                ...
                var subset = spell.Ambiguity.FindAll(test =>
                    test.Target == (int)SpellTarget.SELF
                    && spellClass != 0
                    && (test.ClassMask & spellClass) == spellClass
                );
        ...
        private static List<ActionBlock> SearchActions(
            List<ActionBlock> allActions,
            double beginTime,
            double endTime
        )
        {
        ...
        public static SpellTreeResult SearchSpellPath(
            SpellTreeNode node,
            List<string> data,
            int lastIndex = -1
        )
        {
            ...
            if (node.Words.TryGetValue(data[lastIndex], out SpellTreeNode child))
            {
                ...
                {
                    return new SpellTreeResult
                    {
                        SpellData = child.SpellData,
                        DataIndex = lastIndex,
                    };
                }
            }
        ...
        private static void BuildSpellPath(
            List<string> data,
            SpellTreeNode node,
            SpellData spellData,
            int lastIndex = -1
        ...
        static int DurationCompare(SpellData a, SpellData b)
        {
            ...
            {
                if (
                    int.TryParse(a.ID, out int aInt)
                    && int.TryParse(b.ID, out int bInt)
                    && aInt != bInt
                )
                {
        ...
        private class TimedActionComparer : IComparer<TimedAction>
        {
            public int Compare(TimedAction x, TimedAction y) =>
                (x != null && y != null) ? x.BeginTime.CompareTo(y.BeginTime) : 0;
        }
        ...
        public class SpellTreeNode
        {
            ...
            internal Dictionary<string, SpellTreeNode> Words { get; set; } =
                new Dictionary<string, SpellTreeNode>();
        }
        ...
        public sealed class THJSpellSlot
        {
            ...
            public override string ToString()
            {
                return String.Format(
                    "SPA {0} Base1={1} Base2={2} Max={3} Calc={4}",
                    SPA,
                    Base1,
                    Base2,
                    Max,
        ...
        public static partial class THJSpellParser
        {
            ...
            public static int GetLowestLevel(THJSpell s)
            {
                ...
                for (int i = 0; i < 16; i++)
                    min =
                        ((int)s.Levels[i] > 0 && s.Levels[i] != 255)
                            ? Math.Min((int)s.Levels[i], min)
                            : min;
                return min == -1 ? 0 : min;
            }
        }
