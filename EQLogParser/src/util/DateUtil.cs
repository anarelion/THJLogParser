...
namespace EQLogParser
{
    internal class DateUtil
    {
        ...
        internal static string FormatSimpleDate(double seconds) =>
            new DateTime()
                .AddSeconds(seconds)
                .ToString("MMM dd HH:mm:ss", CultureInfo.InvariantCulture);

        internal static string FormatSimpleHMS(double seconds) =>
            new DateTime().AddSeconds(seconds).ToString("HH:mm:ss", CultureInfo.InvariantCulture);

        internal static string FormatSimpleMS(double seconds) =>
            new DateTime().AddSeconds(seconds).ToString("mm:ss", CultureInfo.InvariantCulture);
        ...
        internal static double ToDouble(DateTime dateTime) =>
            dateTime.Ticks / TimeSpan.FromSeconds(1).Ticks;

        internal static DateTime FromDouble(double value) =>
            new DateTime((long)value * TimeSpan.FromSeconds(1).Ticks);
        ...
        private static DateTime CustomDateTimeParser(
            string dateFormat,
            string source,
            int offset = 0
        )
